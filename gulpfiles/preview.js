...
function fileInclude() {
  return gulp
    ...
    .pipe(replace(/<!-- bundlecss:vendor \[(.*?)\](.*)-->/g, function (math, p1, p2) {
      return `<link rel="stylesheet" href="${p1}/${paths.preview.build.css}/${config.fileNames.build.vendorCSS}${p2.trim()}">`;
    }))
    .pipe(replace(/<!-- bundlecss:theme \[(.*?)\](.*)-->/g, function (math, p1, p2) {
      return `<link rel="stylesheet" href="${p1}/${paths.preview.build.css}/${config.fileNames.build.css}${p2.trim()}">`;
    }))
    .pipe(replace(/<!-- bundlejs:vendor \[(.*?)\](.*)-->/g, function (math, p1, p2) {
      return `<script src="${p1}/${paths.preview.build.js}/${config.fileNames.build.vendorJS}${p2.trim()}"></script>`;
    }))
    .pipe(replace(/<!-- bundlejs:theme \[(.*?)\](.*)-->/g, function (math, p1, p2) {
      return `<script src="${p1}/${paths.preview.build.js}/${config.fileNames.build.js}${p2.trim()}"></script>`;
    }))
...
gulp.task('preview', gulp.series(clean, fileInclude, buildCSS, buildVendorCSS, buildJS, buildVendorJS, svgCompiler, copeImages, copySkippedFiles, copySkippedNodeFiles, copyDependencies));
